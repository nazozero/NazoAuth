...
仓库中保留了对应的工程证据：[2026-06-09 OIDF full matrix](docs/conformance/2026-06-09-oidf-full-matrix.md)。移除 OIDF-only 前端页面并启用 JSON-only 后端授权错误响应后，最新官方 full matrix 记录为 [2026-06-13 real public UI OIDF regression](docs/conformance/2026-06-13-real-public-ui-regression.md) 中的 workflow run `27491182262`。该 run 针对 `https://auth.nazo.run` 执行，GitHub conclusion 为 `success`，官方 runner 报告 71 个 test modules、6375 successes、`0 failures`、`0 warnings`。
