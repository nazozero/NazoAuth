...
| Browser-based clients / SPA / BFF | NazoAuthWeb uses a same-origin server-session/BFF boundary and stores no OAuth credentials. Third-party browser applications are public clients using authorization code + S256 PKCE, no implicit, no access token in the authorization response, strict redirect/origin policy, policy-gated rotating refresh tokens, and minimal non-credentialed CORS. | OAuth browser-based apps draft-26, OAuth 2.1 draft, RFC 9700 | Implemented architecture guidance; not a final-RFC profile claim. | `2026-07-11-browser-based-applications-draft-26-audit.md`, browser route/CORS, PKCE, redirect, refresh, session and CSRF tests | Arbitrary third-party token storage remains application responsibility; repeat the audit after the final RFC is published. |
...
| OAuth 2.0 for Browser-Based Applications draft | BCP guidance for SPA/browser OAuth clients; draft 26 is in the RFC Editor queue without an RFC number on 2026-07-11. | Draft-26 audit complete; publication watch remains | NazoAuthWeb uses the BFF/session architecture; public browser clients use code + S256 PKCE and endpoint-specific non-credentialed CORS. Re-audit every requirement after RFC publication and do not invent a draft runtime profile. |
