...
| RFC 9865 cursor pagination | Adjacent SCIM listing capability, not OAuth/OIDC/FAPI core. | Selected for separate design/not implemented | Keep `cursor: false` until opaque, tenant/filter/count-bound cursor validation and negative tests are implemented. |
