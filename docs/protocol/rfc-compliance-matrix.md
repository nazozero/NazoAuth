...
| JAR | Accept signed request objects where enabled; require signed JAR for the FAPI Message Signing authorization-request option. The authorization endpoint requires an outer `client_id`, including with PAR handles; it must match the Request Object or pushed request. PAR follows RFC 9126 section 3 and can resolve a client from authentication parameters or the Request Object. Validate issuer/client, audience, expiry, not-before, signature, alg allowlist, key status, and parameter conflict rules. | RFC 9101 sections 5–6, RFC 9126 sections 3–4, FAPI2 Message Signing, RFC 8725 | Required for FAPI message-signing authorization-request option; optional in baseline. | `crates/authorization-server/src/authorization/jar.rs` and JAR/PAR tests | External `request_uri` by-reference fetching is deferred because it needs SSRF, cache, lifetime, trust, and content-type controls. |
| Request object replay | Reject signed request-object replay when `jti` is supplied; allow stricter deployments to require `jti`. | RFC 9101, FAPI2 hardening | Recommended; configurable strict policy exists. | `crates/authorization-server/src/authorization/jar.rs`, JAR validation and replay cache tests | For high-value production, prefer requiring signed request-object `jti`. |
...
| Browser-based clients / SPA | NazoAuthWeb is the same-origin authorization-server frontend, uses a server-managed login session, and stores no OAuth credentials; it is not a resource-proxying BFF. Third-party browser applications are public clients using authorization code + S256 PKCE, no implicit, no access token in the authorization response, strict redirect/origin policy, policy-gated rotating refresh tokens, and minimal non-credentialed CORS. | OAuth browser-based apps draft-27, OAuth 2.1 draft, RFC 9700 | Implemented AS/frontend guidance; not a final-RFC or BFF profile claim. | Browser route/CORS, PKCE, redirect, refresh, session, and CSRF tests | Arbitrary third-party architecture and token storage remain application responsibility; the final RFC 10017 delta audit remains pending; see the source-freshness review. |
