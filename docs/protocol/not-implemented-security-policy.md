...
| OIDC Hybrid OP | **Not implemented by security policy.** Front-channel `id_token`/token responses expand leakage and mix-up surfaces without serving the supported code-flow profiles. OAuth 2.1 and the RFC 9700 posture use authorization code plus PKCE. |
