...
证据，不表示候选协议已经实现或通过认证。RFC 9865 被选入独立、受限的 SCIM
cursor pagination 设计；OpenID4VCI / OpenID4VP 需要单独产品立项；其余候选项继续
deferred，直到各自证据记录中的 re-entry 条件满足。
...
| 当前优先缺口 | M8 治理门禁已完成；RFC 9865 进入独立设计，其余候选项保持 deferred 或等待单独产品立项。 |
