...
| Dynamic Client Registration | Complete | Advertised only while the DCR module is active with a non-empty initial access token | [RFC 7591](https://www.rfc-editor.org/rfc/rfc7591.html), [OpenID Connect Dynamic Client Registration 1.0](https://openid.net/specs/openid-connect-registration-1_0.html) | DCR-created clients receive baseline policy and cannot self-grant elevated capabilities. |
...
| JWT Client Authentication and Authorization Grants | Supported; JWT bearer grant is bounded | `private_key_jwt` is available; the JWT bearer grant is limited to authenticated confidential-client self-assertion | [RFC 7523](https://www.rfc-editor.org/rfc/rfc7523.html) | Does not implement third-party JWT assertion issuer trust, arbitrary subject mapping, or issuer trust chains. That is separate from the configured external OIDC, OAuth2 social, and SAML interactive-login adapters. |
...
| OIDC Implicit OP | Never supported | No enablement switch; not advertised | [OpenID Connect Core 1.0](https://openid.net/specs/openid-connect-core-1_0.html), [RFC 9700 Section 2.1.2](https://www.rfc-editor.org/rfc/rfc9700.html#section-2.1.2), [OAuth 2.1 draft](https://datatracker.ietf.org/doc/draft-ietf-oauth-v2-1/) | NazoAuth excludes all implicit responses by product policy; RFC 9700 addresses front-channel access-token issuance specifically. |
| OIDC Hybrid OP | Never supported | No enablement switch; not advertised | [OpenID Connect Core 1.0](https://openid.net/specs/openid-connect-core-1_0.html), [RFC 9700 Section 2.1.2](https://www.rfc-editor.org/rfc/rfc9700.html#section-2.1.2), [OAuth 2.1 draft](https://datatracker.ietf.org/doc/draft-ietf-oauth-v2-1/) | Excluded by [NazoAuth product policy](../protocol/not-implemented-security-policy.md), which is stricter than RFC 9700 for `code id_token`. |
...
| --- | --- | --- | --- |
| OAuth 2.1 Authorization Framework | Draft-15 implementation basis; draft-16 audit pending | [draft-ietf-oauth-v2-1-16](https://datatracker.ietf.org/doc/draft-ietf-oauth-v2-1/) is the current tracked draft. Earlier draft-15 evidence does not establish draft-16 or final-RFC compliance. | Review the delta, including value limits, PKCE and consent guidance, before extending the claim. |
| OAuth 2.0 for Browser-Based Applications | Final-RFC audit pending | [RFC 10017](https://www.rfc-editor.org/rfc/rfc10017.html) is published. The earlier draft-27 review does not establish final-RFC compliance. | Review SPA, BFF, browser-client and session guidance against the RFC; no separate runtime profile is implied. |
| Cross-Device Flows: Security BCP | Audit pending | [RFC 10027](https://www.rfc-editor.org/rfc/rfc10027.html) is published. Device Grant, CIBA, and Native SSO exist but have not completed a requirement-by-requirement audit against this BCP. | Review each applicable cross-device flow without expanding grant authority. |
...
| JWT BCP / JWT Assertion bis | Not supported (audit pending) | [draft-ietf-oauth-rfc8725bis-10](https://datatracker.ietf.org/doc/draft-ietf-oauth-rfc8725bis/) and [draft-ietf-oauth-rfc7523bis-11](https://datatracker.ietf.org/doc/draft-ietf-oauth-rfc7523bis/) are not final yet. Current behavior supports RFC 8725/RFC 7523, not these bis drafts. | Re-audit algorithm allowlists, audience, replay, key binding, cross-JWT confusion, and `private_key_jwt`. |
...
| RAR / Resource Metadata extensions | Watch only | [draft-ietf-oauth-rar-metadata-remediation-00](https://datatracker.ietf.org/doc/draft-ietf-oauth-rar-metadata-remediation/), [draft-skokan-oauth-resource-response-02](https://datatracker.ietf.org/doc/draft-skokan-oauth-resource-response/), and [draft-mcguinness-oauth-rfc9728bis-01](https://datatracker.ietf.org/doc/draft-mcguinness-oauth-rfc9728bis/) may improve typed permissions and protected-resource metadata. |
...
| Dynamic registration | `/register` | Advertised only while the DCR module is active with a non-empty initial access token. |
...
| Hybrid Code + ID Token | Never supported | `code id_token` | [OpenID Connect Core](https://openid.net/specs/openid-connect-core-1_0.html), [RFC 9700 Section 2.1.2](https://www.rfc-editor.org/rfc/rfc9700.html#section-2.1.2) | Excluded by [NazoAuth product policy](../protocol/not-implemented-security-policy.md), which is stricter than RFC 9700 for `code id_token`. |
...
NazoAuth policy excludes implicit and hybrid responses. The normative and
product-specific reasons are distinguished below.
...
## Standards and product-policy boundaries

These implementation boundaries combine normative requirements with stricter
product choices. A linked standard does not imply it prohibits every value
that NazoAuth excludes; the table states the applicable reason.
...
These are explicit implementation policies, with the standards basis described
above; they are not missing configuration switches. Do not attempt to enable them with hidden deployment options.
