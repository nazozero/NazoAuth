...
| OAuth 2.0 for Browser-Based Applications | Draft-27 audit complete; final RFC audit pending | [draft-ietf-oauth-browser-based-apps-27](https://datatracker.ietf.org/doc/draft-ietf-oauth-browser-based-apps/) is in the RFC Editor queue. Current behavior already uses code + S256 PKCE, rejects implicit, avoids browser front-channel token delivery, and has a draft-27 delta audit. | Re-audit SPA, BFF, and browser-client guidance after RFC publication; do not add a false runtime profile or discovery claim. |
| Cross-Device Flows: Security BCP | Related flows supported; final BCP audit pending | [draft-ietf-oauth-cross-device-security-16](https://datatracker.ietf.org/doc/draft-ietf-oauth-cross-device-security/) is a BCP for multiple flows, not a single endpoint. Device Grant, CIBA, and Native SSO exist as separate modules. | Apply the final BCP to those cross-device flows; do not automatically expand grant types. |
| OAuth Security BCP Update | RFC 9700 supported; update draft audit pending | [draft-ietf-oauth-security-topics-update-03](https://datatracker.ietf.org/doc/draft-ietf-oauth-security-topics-update/) is an update track for [RFC 9700](https://www.rfc-editor.org/rfc/rfc9700.html), not a new capability. | Treat it as an RFC 9700 delta audit; update behavior, metadata, tests, and documentation together. |
| JWT BCP / JWT Assertion bis | Current RFCs supported; bis draft audit pending | [draft-ietf-oauth-rfc8725bis-06](https://datatracker.ietf.org/doc/draft-ietf-oauth-rfc8725bis/) and [draft-ietf-oauth-rfc7523bis-11](https://datatracker.ietf.org/doc/draft-ietf-oauth-rfc7523bis/) are not final yet; current behavior follows RFC 8725/RFC 7523 for JWT allowlists, `private_key_jwt`, and the bounded JWT bearer grant. | Re-audit algorithm allowlists, audience, replay, key binding, cross-JWT confusion, and `private_key_jwt`. |
| OAuth Client Attestation | Supported; disabled by default; final RFC audit pending | [draft-ietf-oauth-attestation-based-client-auth-10](https://datatracker.ietf.org/doc/draft-ietf-oauth-attestation-based-client-auth/) is still a draft. The `attest_jwt_client_auth` method is supported only when the Client Attestation module is enabled and client policy requires it. | After final RFC publication, re-audit challenge/freshness, key binding, trust store, replay, revocation, DPoP/refresh interaction, and downgrade tests. |
