...
| Done | [FAPI CIBA Profile](https://openid.net/specs/openid-financial-api-ciba.html) | CIBA token issuance reuses profile validation, confidential client authentication, registered grant allowlists, and DPoP/mTLS sender constraints for clients that require them. | Keep FAPI-CIBA conformance matrix aligned with runtime feature gates; add request-signing-specific cases if target profile requires signed authentication requests. |
...
| Done | [RFC 7523 JWT Bearer Authorization Grant](https://www.rfc-editor.org/rfc/rfc7523) | Implemented as a bounded confidential-client self-assertion grant using registered client JWKs, issuer audience, expiry/iat/nbf limits, and jti replay protection. | Keep third-party assertion issuer trust as separate work requiring issuer allowlist, subject mapping, and audit policy. |
...
| Partial | [OpenID Connect Federation 1.0](https://openid.net/specs/openid-connect-federation-1_0.html) | `/.well-known/openid-federation` can publish a signed self-issued entity statement behind `ENABLE_OIDC_FEDERATION=true`. | Complete trust anchor configuration, trust chain resolution, metadata policy processing, trust marks, and federation fetch/list/resolve before claiming full Federation OP/RP support. |
