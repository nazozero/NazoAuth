...
| P2 | [RFC 9701, JWT Response for OAuth Token Introspection](https://www.rfc-editor.org/rfc/rfc9701) | `src/http/token/introspect.rs` returns JSON introspection responses. No signed JWT introspection response mode or metadata was found. | Add content negotiation or explicit request parameter support, signed JWT response generation, audience/issuer binding, key selection, and negative tests for response tampering assumptions. |
