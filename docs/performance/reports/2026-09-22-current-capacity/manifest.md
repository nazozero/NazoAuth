...
`sha256` (`/proc/1/exe`), `MIGRATION_SET_SHA256`, `PG_SCHEMA_SHA256`, PostgreSQL
runtime settings, Valkey version/policy, harness versions, host profile.
...
| `PG_SCHEMA_SHA256` | capture-time-sensitive — see note below |
...
`PG_SCHEMA_SHA256` is a `pg_dump -s` hash. It varies between captures
(`19bd89af…`, `0cdee983…`, `974a2558…` observed on the same migration set)
because runtime-module state tables are materialized by the app at first
boot and `pg_dump` orders objects by OID — identical logical schema can dump
in different physical order. Stable schema identity =
`MIGRATION_SET_SHA256` + `applied-migrations.txt` (84 rows).
...
- `evidence/current-capacity-final-30m-fresh/` — canonical sustained run on fresh DB (same soak evidence set)
