...
| --- | --- | --- | --- | --- | --- |
| Grant types | Support authorization code, refresh token, client credentials, the bounded RFC 7523 JWT bearer grant, bounded RFC 8693 Token Exchange, and RFC 8628 Device Authorization Grant only when `ENABLE_DEVICE_AUTHORIZATION_GRANT=true` and the client registration includes `urn:ietf:params:oauth:grant-type:device_code`. Do not support implicit or resource owner password grants. CIBA and DCR remain absent unless explicitly implemented with their own threat model. | OAuth 2.1 draft, RFC 7523, RFC 8628, RFC 8693, RFC 9700, FAPI2 | Required for current profile; device grant is optional and default-closed. | `src/http/token/dispatch.rs`, `src/http/token/device.rs`, `src/http/token/token_exchange.rs`, `src/http/authorization/request.rs`, token dispatch/device/token-exchange tests | Keep discovery metadata limited to implemented grants, runtime feature gates, and per-client grant allowlists. |
