...
| --- | --- | --- | --- | --- | --- |
| Grant types | Support authorization code, refresh token, client credentials, and the bounded RFC 7523 JWT bearer grant. Do not support implicit or resource owner password grants. Device Authorization Grant, Token Exchange, CIBA, and DCR remain absent unless explicitly implemented with their own threat model. | OAuth 2.1 draft, RFC 7523, RFC 9700, FAPI2 | Required for current profile; deferred grants remain unadvertised. | `src/http/token/dispatch.rs`, `src/http/authorization/request.rs`, token dispatch tests | Keep discovery metadata limited to implemented grants and per-client grant allowlists. |
