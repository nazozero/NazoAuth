...
| RFC 7523 | `private_key_jwt` is required for FAPI; JWT bearer authorization grant is implemented only for authenticated confidential clients asserting their own `client_id`. | Partial/bounded | Third-party assertion trust and arbitrary subject mapping are not implemented. |
...
| RFC 8693 | Powerful but high-risk delegation/impersonation surface. | Partial/bounded local profile | Current implementation accepts only locally issued access tokens as subject/actor tokens, issues only local access tokens, requires explicit target, downscopes scopes, validates revocation, preserves delegated actor context with `act`, and does not implement external token trust, refresh-token exchange, ID-token exchange, or the full RFC 8693 token-type matrix. |
...
| OIDC Dynamic Client Registration | Useful but high-risk provisioning surface. | Partial/default-closed | Advertise only when `ENABLE_DYNAMIC_CLIENT_REGISTRATION=true`; protect public deployments with `DYNAMIC_CLIENT_REGISTRATION_INITIAL_ACCESS_TOKEN`; support RFC 7592 management only for DCR-created clients with hashed registration access tokens; software statements and remote `jwks_uri` fetching are not implemented. |
