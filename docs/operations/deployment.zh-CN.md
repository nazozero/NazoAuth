...
| 前端 UI | `https://issuer.example/ui/` |
| Health | `https://issuer.example/health` |
...
| Angie worker | `www` |
| 公网 UI 探针 | `https://issuer.example/ui/auth` 及其引用的一个 `/ui/assets/...` 制品 |
...
该脚本通过指定的 SSH 目标部署 `issuer.example` 环境。迁移到其他主机前，必须重新检查监听器、反向代理、容器网络、TLS 设置和 expected issuer。

### 固定内网 IP 与 Angie 反代

`issuer.example` live 路径固定使用 Podman bridge 网络 `nazo_oauth_net`、subnet
...
curl -fsS https://issuer.example/.well-known/oauth-authorization-server
curl -fsS https://issuer.example/.well-known/oauth-protected-resource
curl -fsS https://issuer.example/.well-known/oauth-protected-resource/fapi/resource
curl -fsS https://issuer.example/jwks.json
```
