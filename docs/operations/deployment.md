...
Use the mandatory [public black-box conformance runbook](../conformance/oidf-public-black-box-runbook.md).
The release deployment installs only the normal product image and UI. Client
records, OpenID4VC datasets, and mTLS trust anchors must be created through the
public production control plane after deployment; release code must not write
them by SQL or consume runner-generated CA material.

The reverse proxy may install only the bundle exported from the approved mTLS
trust-anchor control plane. Preserve its digest and rollback copy, validate the
complete proxy configuration, and verify the public mTLS endpoint after reload.
The repository never supplies a default public issuer or suite origin.
