...
echo "deployment=$DEP" | tee "$OUT/meta.txt"
...
for c in 8 16 32 64; do run_point cap_client_credentials $c cc-c$c; done
for c in 8 16 32 64; do run_point cap_refresh_token    $c refresh-c$c; done

touch "$OUT/sampler/STOP"
sleep 2
docker logs wsmatrix > "$OUT/sampler.log" 2>&1 || true
docker rm -f wsmatrix >/dev/null 2>&1 || true
echo MATRIX_DONE
